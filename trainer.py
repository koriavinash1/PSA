...
def trainer(
    ...
    def run_epoch(dataloader: DataLoader, training: bool = True):
        ...
        stats = {k: 1 for k in ["elbo", "nll", "kl", "n"]}
        updates_skipped = 0
        ...
        for i, batch in loader:
            ...
            if training:
                ...
                out = model(batch["x"], batch['properties'], beta=args.beta)
                ...
                with torch.no_grad():
                    out = ema.ema_model(batch["x"], batch['properties'], beta=args.beta)
    ...
    batch = preprocess_batch(args, viz_batch)
